...
class GaussianDiffusionBeatGans:
    ...
    def __init__(self, conf: GaussianDiffusionBeatGansConfig):
        ...
        self.posterior_mean_coef2 = ((1.0 - self.alphas_cumprod_prev) *
                                     np.sqrt(alphas) /
                                     (1.0 - self.alphas_cumprod))
        self.FLoss=FocalLoss(gamma=2) #test
        self.DICEloss=DiceLoss() #test
    ...
    def training_losses(self,
                        ...
                        cond: th.Tensor = None,
                        ##CHANGED ARIST
                        seg_style: th.Tensor =None,
                        ##CHANGED ARIST
                        label: th.Tensor=None,
                        is_label: th.Tensor=None,
                        model_kwargs=None,
        ...
        if self.loss_type in [
            ...
            with autocast(self.conf.fp16):
                ...
                if seg_style is not None:
                    seg_style=seg_style.detach()
                if cond is not None:
                    cond=cond.detach()
                model_forward = model.forward(x=x_t,
                                              t=self._scale_timesteps(t),
                                              x_start=x_start,
                                              cond=cond, # for the other 
                                              seg_style=seg_style, #for the 2nd
                                              **model_kwargs)   
            ...
            model_label_output=None
            if label is not None:## Modified Arist
                # print("CONTAINS LABEL, UPDATING")
                model_label_output=model_forward.y

            _model_output = model_output
            _model_label_output=model_label_output



            # if self.conf.train_pred_xstart_detach:
            #     _model_output = _model_output.detach()
            #     if model_label_output is not None:
            #         _model_label_output=_model_label_output.detach()
            # get the pred xstart
            ...
            indices_with_lbl=[]
            indices_wo_lbl=[]
            # tmp_dice=th.tensor(0.0)
            # tmp_focol=th.tensor(0.0)
            if label is not None:
                for i in range(len(label)):
                    #Here we need to check if the label only have -1 (if only -1, we will not update the segmentation network)
                    if is_label[i][0]==1:
                        indices_with_lbl.append(i)
                        # print(is_label[i][0],"With Label")
                    else:
                        indices_wo_lbl.append(i)
                        # print(is_label[i][0],"Without Label")
                        # print("Received One Slice With Label {0}".format(i))
                        # #Here we put the label loss
                        # # P = F.softmax(model_label_output, dim=1)
                        # # lbl_onehot=F.one_hot(label)
                        # tmp_dice=tmp_dice+self.DICEloss.forward(model_label_output[i].unsqueeze(0),label[i].unsqueeze(0))
                        # tmp_focol=tmp_focol+self.FLoss.forward(model_label_output[i].unsqueeze(0),label[i].unsqueeze(0))
                # weights_loss=[1 if label[i].max()!=-1 else 0 for i in range(len(label))]
                # breakpoint()

                terms["DICE_loss"]=th.tensor(0.0).to(model_label_output.device)
                terms["focol_loss"]=th.tensor(0.0).to(model_label_output.device)
                
                is_label=is_label.to(model_label_output.device)
                # for i in range(len(label)):
                #     tmp_dice=self.DICEloss.forward(model_label_output[i].unsqueeze(0),label[i].unsqueeze(0))
                #     tmp_focol=self.FLoss.forward(model_label_output[i].unsqueeze(0),label[i].unsqueeze(0))
                #     terms["DICE_loss"]=terms["DICE_loss"]+tmp_dice*is_label[i][0]
                #     terms["focol_loss"]=terms["focol_loss"]+tmp_focol*is_label[i][0]
                # if is_label.sum()!=0:
                #     terms["DICE_loss"]/=is_label.sum()
                #     terms["focol_loss"]/=is_label.sum()
                # breakpoint()
                if indices_with_lbl==[]: #Empty list return reasonable outpu
                    terms["DICE_loss"]=self.DICEloss.forward(model_label_output[indices_with_lbl],label[indices_with_lbl])*0.0+self.DICEloss.forward(model_label_output[indices_wo_lbl],label[indices_wo_lbl])*0.0
                    terms["focol_loss"]=self.FLoss.forward(model_label_output[indices_with_lbl],label[indices_with_lbl])*0.0+self.FLoss.forward(model_label_output[indices_wo_lbl],label[indices_wo_lbl])*0.0
                else:
                    terms["DICE_loss"]=self.DICEloss.forward(model_label_output[indices_with_lbl],label[indices_with_lbl])+self.DICEloss.forward(model_label_output[indices_wo_lbl],label[indices_wo_lbl])*0.0
                    terms["focol_loss"]=self.FLoss.forward(model_label_output[indices_with_lbl],label[indices_with_lbl])+self.FLoss.forward(model_label_output[indices_wo_lbl],label[indices_wo_lbl])*0.0
                # print(terms["DICE_loss"],terms["focol_loss"])
                # terms["loss"]=th.tensor(0.0).to(model_label_output.device)
                '''
                if len(indices_with_lbl)==0:
                    terms["DICE_loss"]=th.tensor(0.0)
                    terms["focol_loss"]=th.tensor(0.0)
                else:
                    # breakpoint()
                    terms["DICE_loss"]=self.DICEloss.forward(model_label_output[indices_with_lbl],label[indices_with_lbl])
                    terms["focol_loss"]=self.FLoss.forward(model_label_output[indices_with_lbl],label[indices_with_lbl]) #Here we only select those who have label information to update
                '''
            ...
            # for i in range(len(label)):
            #     terms["loss"]=terms["loss"]+terms["mse"]+terms["DICE_loss"][i]
            if label is not None:
                terms["loss"]= terms["mse"]+terms["DICE_loss"]*0.5+terms["focol_loss"]*0.5
        else:
    ...
    def p_mean_variance(self,
        ...
        # --- 1) UNet forward만 checkpoint로 감쌈 ---
        with autocast(self.conf.fp16):
            def run_unet(x_in):
                # model.forward 는 (pred, y, …) 같은 튜플 또는 Namespace를 반환합니다.
                mf = model.forward(
                    x=x_in,
                    t=self._scale_timesteps(t),
                    seg_style=seg_style,
                ...
                # 여기서 pred 만 꺼내서 반환
                return mf.pred, mf  # 또는 (mf.pred, mf) 로 튜플 형태
            pred, model_forward = checkpoint(run_unet, x)

        model_output = pred

        # --- 2) fixed variance 계산 (변경 없음) ---
        if self.model_var_type in [ModelVarType.fixed_large, ModelVarType.fixed_small]:
            ...
            # learnable variance 등 다른 경우 처리...
            pass

        # --- 3) x0 예측 & posterior mean/variance 계산 (변경 없음) ---
        def process_xstart(x0):
            if denoised_fn is not None:
                x0 = denoised_fn(x0)
            return x0.clamp(-1, 1) if clip_denoised else x0
        ...
        return {
            ...
            "model_forward": model_forward,  # 필요하다면
        }
    ...
    def ddim_sample(
        ...
        """
        # posterior mean, posterior variance 및 pred_xstart 등을 계산
        out = self.p_mean_variance(
            model,
            x,
            t,
            cond=cond,#CHANGED ARIST
            seg_style=seg_style,
            clip_denoised=clip_denoised,
            denoised_fn=denoised_fn,
            model_kwargs=model_kwargs,
        ...
        # epsilon, 즉 noise를 원래 직접 예측하는데
        # pred_xstart로부터 e를 역산
        # 이렇게 하면 모델이 x0, x_t-1을 예측했을 때에도 일관된 e계산 가능
        eps = self._predict_eps_from_xstart(x, t, out["pred_xstart"])


        alpha_bar = _extract_into_tensor(self.alphas_cumprod, t, x.shape)    # alphas_cumprod: timestep t까지 누적된 a값값
        alpha_bar_prev = _extract_into_tensor(self.alphas_cumprod_prev, t,   # 바로 이전 timestep a_t-1
                                              x.shape)
        
        # singma(ddim noise scale)계산
        # ddim 식에서 n값에 따라 noise injection 강도 조절
        # n=0이면 deterministic sampling, n>0이면 stochasstic sampling

        sigma = (eta * th.sqrt((1 - alpha_bar_prev) / (1 - alpha_bar)) *
                 th.sqrt(1 - alpha_bar / alpha_bar_prev))
        # Equation 12.
        # 새로운 샘플의 평균(mean_pred)계산산
        noise = th.randn_like(x)
        ...
        return {"sample": sample, "pred_xstart": out["pred_xstart"], "pred_noise": eps, "pred_sigma": sigma}
    ...
    def ddim_sample_loop_progressive(
        ...
        for i in indices:
            ...
            out = self.ddim_sample(
                ...
                t,
                seg_style=seg_style,
                #cond=model_kwargs["cond"], #CHANGED ARIST #This is for first try (basically passing the condition as parameters)
                clip_denoised=clip_denoised,
                denoised_fn=denoised_fn,
                cond_fn=cond_fn,
