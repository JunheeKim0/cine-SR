...
def main():
    ...
    for id in id_list : 
        ...
        vol_sax_orig = ( vol_sax + 1 ) * 2
        # convert to torch
        sax_tensor = torch.from_numpy(vol_sax).float().unsqueeze(1).to(device)    # (Z,1,H,W)
        sax_tensor_orig = torch.from_numpy(vol_sax_orig).float().unsqueeze(1).to(device)    # (Z,1,H,W)
        ...
        # === MODIFICATION 1: original slice inverse optimization setup (batch) ===
        orig_indices   = [i * filling for i in range((x.shape[0] + filling - 1) // filling)]
        sax_indices = [idx // 8 for idx in orig_indices]

        optimizer_orig = torch.optim.Adam([x, c], lr=1e-2)
        scheduler_orig = torch.optim.lr_scheduler.StepLR(optimizer_orig, step_size=5, gamma=0.5)
        scaler_orig    = torch.cuda.amp.GradScaler()

        # batch 처리 대신, 매 반복마다 슬라이싱하여 새로운 그래프 생성
        for e in range(num_epoch_orig):
            optimizer_orig.zero_grad()
            # 매 이터레이션마다 x, c 재슬라이싱
            xi = x[orig_indices]                   # (M,1,H,W)
            ci = c[orig_indices]                   # (M,cond_dim)
            target_o = sax_tensor_orig[sax_indices]    # (M,1,H,W)

            with torch.cuda.amp.autocast():
                out_o    = base_model.render(xi, cond=ci, T=t)  # (M,1,H,W)
                sample_o = (out_o["sample"] + 1) / 2          # (M,1,H,W)
                loss_o   = ((sample_o - target_o) ** 2).mean()

            scaler_orig.scale(loss_o).backward()    # retain_graph 제거
            scaler_orig.step(optimizer_orig)
            scaler_orig.update()
            scheduler_orig.step()
            print(f"[InvEpoch {e+1}/{num_epoch_orig}] MSE loss={loss_o.item():.6f}")
            # --- SSIM & PSNR 계산 추가 ---
            sample_o_np = sample_o.detach().cpu().numpy()
            target_o_np = target_o.detach().cpu().numpy()
            # 슬라이스별 SSIM, PSNR 계산 후 평균
            ssim_vals = [structural_similarity(target_o_np[i,0], sample_o_np[i,0], data_range=1) for i in range(sample_o_np.shape[0])]
            psnr_vals = [peak_signal_noise_ratio(target_o_np[i,0], sample_o_np[i,0], data_range=1) for i in range(sample_o_np.shape[0])]
            mean_ssim = np.mean(ssim_vals)
            mean_psnr = np.mean(psnr_vals)
            print(f"[InvEpoch {e+1}/{num_epoch_orig}] SSIM={mean_ssim:.4f} PSNR={mean_psnr:.2f}")


        # inverse 최적화된 slice는 메인 최적화에서 고정
        active[orig_indices] = False
        # === END MODIFICATION 1 ===
        ...
        def mask_gradients(grad):
            grad = grad.clone()
            grad[~active] = 0
            grad[orig_indices] = 0                            # MODIFICATION 2: original slice gradient 고정
            return grad
        ...
        for epoch in range(num_epoch):
            with torch.cuda.amp.autocast():
                ...
                # 3) 양측 smooth loss (Discrete Laplacian)
                center      = vol_pred[1:-1]
                prev        = vol_pred[:-2]
                next_slice  = vol_pred[2:]
                laplacian   = 2*center - prev - next_slice
                smooth_loss = laplacian.pow(2).mean()
        ...
        save_path_a = os.path.join(
            save_dir_a,
            f"MR_Heart_{sid}_frame_{frame_idx}_seperate_ori_{new_n}slice.nii.gz"
        )
