...
def main():
    ...
    for id in id_list : 
        ...
        gt_tensor  = torch.from_numpy(gt_vol_norm).float().unsqueeze(1).to(device)      # (Z,1,H,W)
        ...
        for epoch in range(num_epoch):
            ...
            for i in range(0, len(idxs), chunk_size):
                ...
                pred_norm_roi = pred_norm * mask_vol
                ...
                print("my_interp_xTs.grad:", my_interp_xTs.grad.norm())         
            ...
            # # --- Langevin update: 전체 latent에 대해 한 번만 적용 ---
            # with torch.no_grad():
            #     # (1) 전체 latent에 대한 score 계산
            #     pred_score_all = base_model.render(
            #         my_interp_xTs, my_interp_cs, t)
            #     σ = pred_score_all["pred_sigma"].clamp(min=1e-3)
            #     ε = pred_score_all["pred_noise"]
            #     score_all = - ε / σ                              # score ≈ -ε/σ
            #     η = 1e-3

            #     # (2) 전체 latent 업데이트
            #     my_interp_xTs.data += η * score_all
