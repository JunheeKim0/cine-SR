...
def main():
    ...
    lambda_roi    = 100.0  
    lambda_latent = 10   
    lambda_smooth = 100.0         # 초기 smooth 가중치
    lambda_prior  = 1.0           # 초기 prior  가중치
    target_smooth_ratio = 0.1     # smooth_term / total_loss 의 목표 비율 (10%)
    ...
    for id in id_list : 
        ...
        for epoch in range(num_epoch):
            ...
            total_loss  = lambda_roi * total_mse_loss + smooth_term + prior_term
        ...
        save_path_a = os.path.join(
            save_dir_a,
            f"MR_Heart_{sid}_frame_{frame_idx}_slice_schedule_{new_n}slice.nii.gz"
        )
