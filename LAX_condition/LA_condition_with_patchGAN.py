...
def main():
    ...
    for id in id_list : 
        ...
        optim_G        = AdamW8bit([my_interp_xTs, my_interp_cs], lr=1e-2, weight_decay=1e-2)
        sched_G        = StepLR(optim_G, step_size=50, gamma=0.5)
        sched_D        = StepLR(optim_D, step_size=50, gamma=0.5)

        scaler         = GradScaler()

        # --- 6) Training Loop (전체 loss) ---
        for epoch in range(num_epoch):
            ### (1) Generator(fake) Forward
            with autocast():
                pred_all = base_model.render(my_interp_xTs, my_interp_cs, t)
                samples  = (pred_all["sample"] + 1) / 2    # (N,1,H,W)
            vol_pred = samples.squeeze(1)                # (N,H,W)

            # — projection & affine normalize —
            sax_2ch_fake = projection_torch(vol_pred, mask_2ch)
            sax_3ch_fake = projection_torch(vol_pred, mask_3ch)
            sax_4ch_fake = projection_torch(vol_pred, mask_4ch)

            sax_2ch_fake = affine_normalize(sax_2ch_fake, proj_gt_2ch).unsqueeze(0).unsqueeze(0)
            sax_3ch_fake = affine_normalize(sax_3ch_fake, proj_gt_3ch).unsqueeze(0).unsqueeze(0)
            sax_4ch_fake = affine_normalize(sax_4ch_fake, proj_gt_4ch).unsqueeze(0).unsqueeze(0)

            sax_2ch_real = proj_gt_2ch.unsqueeze(0).unsqueeze(0)
            sax_3ch_real = proj_gt_3ch.unsqueeze(0).unsqueeze(0)
            sax_4ch_real = proj_gt_4ch.unsqueeze(0).unsqueeze(0)

            ### (2) Discriminator 업데이트
            # 2ch
            D_real_2ch = D(sax_2ch_real, sax_2ch_real)
            D_fake_2ch = D(sax_2ch_real, sax_2ch_fake.detach())
            valid      = torch.ones_like(D_real_2ch)
            fake       = torch.zeros_like(D_real_2ch)
            loss_D_2ch = 0.5 * (
                criterion_adv(D_real_2ch, valid) +
                criterion_adv(D_fake_2ch, fake)
            )

            # 3ch
            D_real_3ch = D(sax_3ch_real, sax_3ch_real)
            D_fake_3ch = D(sax_3ch_real, sax_3ch_fake.detach())
            loss_D_3ch = 0.5 * (
                criterion_adv(D_real_3ch, valid) +
                criterion_adv(D_fake_3ch, fake)
            )

            # 4ch
            D_real_4ch = D(sax_4ch_real, sax_4ch_real)
            D_fake_4ch = D(sax_4ch_real, sax_4ch_fake.detach())
            loss_D_4ch = 0.5 * (
                criterion_adv(D_real_4ch, valid) +
                criterion_adv(D_fake_4ch, fake)
            )

            loss_D = loss_D_2ch + loss_D_3ch + loss_D_4ch
            optim_D.zero_grad()
            loss_D.backward()
            optim_D.step()
            sched_D.step()

            ### (3) Generator(latent) 업데이트
            adv_loss_2ch = criterion_adv(D(sax_2ch_real, sax_2ch_fake), valid)
            adv_loss_3ch = criterion_adv(D(sax_3ch_real, sax_3ch_fake), valid)
            adv_loss_4ch = criterion_adv(D(sax_4ch_real, sax_4ch_fake), valid)
            loss_G_adv   = adv_loss_2ch + adv_loss_3ch + adv_loss_4ch

            # L1 loss
            l1_2ch       = F.l1_loss(sax_2ch_fake.squeeze(1), proj_gt_2ch, reduction='sum') / mask_2ch.sum().float()
            l1_3ch       = F.l1_loss(sax_3ch_fake.squeeze(1), proj_gt_3ch, reduction='sum') / mask_3ch.sum().float()
            l1_4ch       = F.l1_loss(sax_4ch_fake.squeeze(1), proj_gt_4ch, reduction='sum') / mask_4ch.sum().float()
            total_mse_loss= l1_2ch + l1_3ch + l1_4ch
            ...
            if local_rank == 0:
                print(f"[Epoch {epoch+1}/{num_epoch}] "
                    f"D_loss={loss_D.item():.6f}, G_loss={total_G_loss.item():.6f}", f"mse_loss={total_mse_loss}")
        ...
        save_path_a = os.path.join(
            save_dir_a,
            f"MR_Heart_{sid}_frame_{frame_idx}_gan_{new_n}slice.nii.gz"
        )
