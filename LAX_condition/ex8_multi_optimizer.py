...
# 843
# 834, 148, 400, 769, 243, 813, 600, 100, 708,
...
def main(local_rank, world_size):
    ...
    for sid in my_ids:
     ...
     ################### ======== Slice별 최적화 시작 (new_n 개수만큼) ========= ######################
            ...
            gt_roi_batch     = region_gt_batch * region_mask_batch
            mu_gt             = gt_roi_batch.view(new_n, -1).sum(dim=1) / n_valid_batch
            sigma_gt          = torch.sqrt(
                ((gt_roi_batch - mu_gt[:,None,None,None]) * region_mask_batch)
                .view(new_n, -1).pow(2).sum(dim=1) / n_valid_batch + 1e-6
            )
            ...
            for epoch in range(num_epochs):
                ...
                with torch.cuda.amp.autocast():
                    ...
                    pred_roi     = pred_batch * region_mask_batch
                    mu_pred      = pred_roi.view(new_n, -1).sum(dim=1) / n_valid_batch
                    sigma_pred   = torch.sqrt(
                        ((pred_roi - mu_pred[:,None,None,None]) * region_mask_batch)
                        .view(new_n, -1).pow(2).sum(dim=1) / n_valid_batch + 1e-6
                    )
                    sigma_pred   = torch.clamp(sigma_pred, min=1e-3)
                    pred_norm    = (
                        (pred_batch - mu_pred[:,None,None,None]) *
                        (sigma_gt[:,None,None,None] / sigma_pred[:,None,None,None])
                        + mu_gt[:,None,None,None]
                    )
                    roi_losses   = F.mse_loss(
                        pred_norm * region_mask_batch,
                        region_gt_batch,
                        reduction="none"
                    ...
                    x_pad           = torch.cat([x_batch[0:1], x_batch, x_batch[-1:]], dim=0)
                    smooth_left     = ((x_batch - x_pad[:-2])
                                        .view(new_n, -1)**2).mean(dim=1)
                    smooth_right    = ((x_batch - x_pad[2:])
                                        .view(new_n, -1)**2).mean(dim=1)
                    smooth_losses   = smooth_left + smooth_right
                ...
                print(
                    f"[Frame {frame_idx}][Epoch {epoch+1}] total_loss={total_loss.item():.6f}")
        ...
        ################### --- 최적화 완료 후 예측 저장 (각 보간된 slice별) --- #######################
            ...
            save_dir_3d = (
                f"/workspace/storage/kjh/dataset/cardiac/output/Inverse/"
