...
# --- 처리할 Subject ID 리스트 ---
# 148, 
#    400, 834, 769, 834, 243, 813, 600, 100, 708,
...
def main(local_rank, world_size):
    ...
    for sid in my_ids:
        ...
        for frame_idx in n_frames:
            ...
            save_dir_before = (
                f"/workspace/storage/kjh/dataset/cardiac/output/DiffAE/"
    ...
    #             # 8) Slice별 latent & condition 초기값 설정 (GPU에 올려 최적화 변수)
    #             x_t_slice_gpu  = interp_xTs_cpu[interp_idx:interp_idx+1].to(device).detach().clone().requires_grad_(True)
    #             cond_slice_gpu = interp_conds_cpu[interp_idx:interp_idx+1].to(device).detach().clone().requires_grad_(True)
    #             x_t_slice_init  = x_t_slice_gpu.detach().clone()  # GPU
    #             cond_slice_init = cond_slice_gpu.detach().clone()  # GPU

    #             # 9) GT 및 Mask for this slice
    #             #    interp_idx를 LAX의 z-index로 사용한다는 가정 하에
    ...
    #             n_valid = region_mask_gpu.sum().float()
    ...
    #             # 11) Gradient Hook 등록 (optional)
    #             def print_grad(name):
    #                 def hook(grad):
    #                     print(f"{name}.grad norm = {grad.norm().item():.6f}")
    #                 return hook

    #             x_t_slice_gpu.register_hook(print_grad("x_t_slice"))
    #             cond_slice_gpu.register_hook(print_grad("cond_slice"))
    ...
    #                             ((pred_roi - mu_pred) * region_mask_gpu).pow(2).sum() / n_valid + 1e-6
    #                         )
    ...
    #                     # 3) Projection: 초기값 쪽으로 살짝 밀어주는 트릭
    #                     with torch.no_grad():
    #                         x_t_slice_gpu.data  = (
    #                             (1 - alpha_proj) * x_t_slice_gpu.data + alpha_proj * x_t_slice_init.data
    #                         )
    #                         cond_slice_gpu.data = (
    #                             (1 - alpha_proj) * cond_slice_gpu.data + alpha_proj * cond_slice_init.data
    #                         )
    ...
    #             save_dir_3d,
    #             f"MR_Heart_{sid}_frame_{frame_idx}_optimized_interp_no_{new_n}slice.nii.gz"
    #         )
