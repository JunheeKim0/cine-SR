...
from torchopt import adam, apply_updates
...
def main():
    ...
    # 148 제외외
    id_list = [
        834, 400, 769, 243, 813, 600, 100, 708,
    ...
    def inter_sax_load(file_path) :
        ...
        # 2) 코드 1과 동일하게 98th percentile 기준으로 클리핑 → [0,1]로 정규화
        mx_sax = np.percentile(vol_np_inter, 98)
        vol_np_inter = np.clip(vol_np_inter, 0, mx_sax) / mx_sax
    ...
    def load_lax_mask(lax_path, mask_path, device):
        ...
        mx_lax = np.nanpercentile(gt_vol, 99)
        gt_vol = np.clip(gt_vol, 0, mx_lax) / mx_lax
    ...
    for id in id_list : 
        ...
        # 0~1 SAX
        vol_sax_opt = ( vol_sax + 1 ) / 2
        sax_tensor_opt = torch.from_numpy(vol_sax_opt).float().unsqueeze(1).to(device)    # (Z,1,H,W)

        # encode + stochastic encode
        cond = ema_model.encode(sax_tensor)
        xTs = ema_model.encode_stochastic(sax_tensor, cond, T=30)

        # --- 2) 시퀀스 생성 ---
        xs, cs = [], []
        N = xTs.shape[0]

        # --- 2) 시퀀스 생성 (코드 1과 동일 α 스텝) ---
        alpha_step = 1.0 / (filling - 1)
        for i in range(N - 1):
            # 시작점
            xs.append(xTs[i])
            cs.append(cond[i])
            # 중간(filling-1)개 α = 1/(filling-1), 2/(filling-1), … 
            for k in range(1, filling):
                alpha = alpha_step * k
                xs.append(slerp(xTs[i], xTs[i+1], alpha))
                cs.append((1 - alpha) * cond[i] + alpha * cond[i+1])
        # 마지막점을 추가
        xs.append(xTs[-1])
        cs.append(cond[-1])
        ...
        # --- 4) slice별 leaf Parameter 생성 (변경 없음) ---
        x_slices = torch.nn.ParameterList([
            torch.nn.Parameter(
                before_interp_xTs[idx:idx+1].clone().detach().to(device),
                requires_grad=True
            )
            for idx in idxs
        ])
        c_slices = torch.nn.ParameterList([
            torch.nn.Parameter(
                before_interp_cs[idx:idx+1].clone().detach().to(device),
                requires_grad=True
            )
            for idx in idxs
        ])
        N = len(x_slices)
        x0_slices = [p.data.clone() for p in x_slices]
        c0_slices = [p.data.clone() for p in c_slices]
        ...
        # --- slice-wise Early Stopping 변수 초기화 ---
        best_loss         = torch.full((N,), float('inf'), device=device)
        epochs_no_improve = torch.zeros(N, dtype=torch.int, device=device)

        # --- 5) 하나의 Optimizer를 slice별 param_group으로 초기화 ---
        param_groups = []
        for i in range(N):
            param_groups.append({
                'params': [x_slices[i], c_slices[i]],
                'slice_idx': i,
                'lr': base_lr,
                'betas': (0.9, 0.999),
                # 필요하면 eps, weight_decay, amsgrad 등도 여기 설정 가능
            })
        optimizer = torch.optim.Adam(param_groups)
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=50, gamma=0.5)

        # --- debug: learning‑rate 출력 함수 ---
        def print_lr(tag):
            lrs = [g['lr'] for g in optimizer.param_groups]
            print(f"[{tag}] current learning rates = {lrs}")

        scaler    = torch.cuda.amp.GradScaler()

        # --- 6) Training Loop (per‑slice) ---
        for epoch in range(num_epoch):
            print_lr(f"Epoch {epoch+1} start")
            # 6-1) 매 epoch마다 살아있는(수렴 안 된) 슬라이스 index 모으기
            alive_idxs = [i for i in range(N) if epochs_no_improve[i] < patience]
            if not alive_idxs:
                # alive slice가 없으면 한 번만 lr decay
                scheduler.step()
                print_lr("After scheduler before break")
                print("All slices converged → stopping.")
                break

            # 6-2) 슬라이스별로 독립적인 forward/backward/step 수행
            for sl_idx in alive_idxs:
                # 1) gradient 초기화
                optimizer.zero_grad()

                # 1) forward & per‑slice affine 정규화
                with torch.cuda.amp.autocast():
                    out       = ema_model.render(
                                   x_slices[sl_idx],
                                   cond=c_slices[sl_idx],
                                   T=t
                               )
                    sample    = (out["sample"] + 1) / 2       # [1,1,H,W]
                    pred      = sample.squeeze(0).squeeze(0)  # [H,W]

                    # 1‑a) 이 슬라이스만 뽑아서 통계 계산
                    mu_sax_i, sigma_sax_i = compute_sax_roi_stats(
                        pred.unsqueeze(0),                                # [1,H,W]
                        mask_vol.squeeze(0)[sl_idx:sl_idx+1],             # [1,H,W]
                        slice_n_valid[sl_idx:sl_idx+1]                    # [1]
                    )
                    # 1‑b) affine normalize
                    sax_norm_i = slicewise_affine_normalize(
                        pred.unsqueeze(0),
                        mask_vol.squeeze(0)[sl_idx:sl_idx+1],
                        mu_sax_i, sigma_sax_i,
                        mu_gt[sl_idx:sl_idx+1], sigma_gt[sl_idx:sl_idx+1]
                    ).squeeze(0)  # [H,W]

                    # 2) per‑slice ROI MSE loss 직접 계산
                    diff_i   = (sax_norm_i - gt_roi_vol[sl_idx]).pow(2) \
                               * mask_vol.squeeze(0)[sl_idx]
                    roi_loss = diff_i.sum() / slice_n_valid[sl_idx]

                    # 3) prior loss
                    latent_reg_i = ((x_slices[sl_idx] - x0_slices[sl_idx])**2).mean()
                    cond_reg_i   = ((c_slices[sl_idx] - c0_slices[sl_idx])**2).mean()
                    prior_loss_i = lambda_latent * (latent_reg_i + cond_reg_i)

                    loss_i = lambda_roi * roi_loss + prior_loss_i
                ...
                # 4) 해당 슬라이스 scheduler.step()
                scheduler.step()

                # 5) Projection Trick (이 슬라이스에 대해서만)
                with torch.no_grad():
                    x_slices[sl_idx].data = (
                        (1 - alpha_proj) * x_slices[sl_idx].data
                        + alpha_proj * x0_slices[sl_idx]
                    )
                    c_slices[sl_idx].data = (
                        (1 - alpha_proj) * c_slices[sl_idx].data
                        + alpha_proj * c0_slices[sl_idx]
                    )

                # 6) Early Stopping 체크
                l = loss_i.item()
                if l + min_delta < best_loss[sl_idx]:
                    best_loss[sl_idx] = l
                    epochs_no_improve[sl_idx] = 0
                else:
                    epochs_no_improve[sl_idx] += 1

                print(
                    f"[Epoch {epoch+1}/{num_epoch}][Slice {sl_idx}] "
                    f"loss={l:.6f} (roi={roi_loss:.6f}, prior={prior_loss_i.item():.6f})"
                )

            # (Optional) Epoch 단위 summary
            print_lr(f"Epoch {epoch+1} end")
        ...
        # --- 최적화된 batch 변수 사용 ---
        opt_xTs = torch.cat([p.detach() for p in x_slices], dim=0)  # (N, latent_dim)
        opt_cs  = torch.cat([p.detach() for p in c_slices], dim=0)  # (N, cond_dim)

        # --- 모델로 렌더링 후 정규화 ---
        with torch.no_grad():
            # optimized 이후
            out_after = ema_model.render(opt_xTs, cond=opt_cs, T=t) 
            pred_after = (out_after["sample"] + 1) / 2                # [-1,1] → [0,1]

            # 최적화 전 (before) 결과도 동일 T로 렌더링
            out_before = ema_model.render(before_interp_xTs, cond=before_interp_cs, T=t)
        ...
        save_path_a = os.path.join(
            save_dir_a,
            f"MR_Heart_{sid}_frame_{frame_idx}_no_smooth_{new_n}slice.nii.gz"
        )
