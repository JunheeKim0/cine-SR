...
def main():
    ...
    for id in id_list : 
        ...
        # --- slice‑wise Early Stopping, gradient hook 초기화 ---
        best_loss         = torch.full((N,), float('inf'), device=device)
        epochs_no_improve = torch.zeros(N, dtype=torch.int, device=device)
        patience          = 10
        active            = torch.ones(N, dtype=torch.bool, device=device)

        def make_mask_hook(idx):
            def hook(grad):
                return torch.zeros_like(grad) if not active[idx] else grad
            return hook

        for i, p in enumerate(x_slices):
            p.register_hook(make_mask_hook(i))
        for i, p in enumerate(c_slices):
            p.register_hook(make_mask_hook(i))

        # --- 5) Functional Optimizer 초기화 (torchopt.adam) ---
        lr, betas = base_lr, (0.9, 0.999)
        func_opt   = adam(lr=lr, betas=betas)      # FuncOptimizer 생성
        opt_states = [
            func_opt.init((x_slices[i], c_slices[i]))
            for i in range(N)
        ]

        scaler = torch.amp.GradScaler(device='cuda')  # AMP GradScaler

        # --- 6) Training Loop (batch‑wise) ---
        for epoch in range(num_epoch):
            # 6‑1) ParameterList → batch Tensor
            x = torch.cat(list(x_slices), dim=0)  # (N,1,H,W)
            c = torch.cat(list(c_slices), dim=0)  # (N,cond_dim)
            ...
            with torch.amp.autocast(device_type='cuda', dtype=torch.float16):
                ...
                n_valid = slice_n_valid
                # 1) per‑slice affine normalization
                mu_sax, sigma_sax = compute_sax_roi_stats(
                    vol_pred, mask_vol.squeeze(0), slice_n_valid
                )
                ...
                # 2) per‑slice ROI MSE loss
                diff           = (sax_norm - gt_roi_vol).pow(2) * mask_vol.squeeze(0)
                per_slice_loss = diff.sum(dim=(1,2)) / slice_n_valid  # (N,)
                total_roi_loss = per_slice_loss.sum()                 # scalar

                # 3) Smooth loss (Discrete Laplacian)
                center     = vol_pred[1:-1]; prev = vol_pred[:-2]; next_ = vol_pred[2:]
                laplacian  = 2*center - prev - next_
                smooth_loss = laplacian.pow(2).mean()                 # scalar

                # 4) Prior loss
                latent_reg = sum(((p - x0_slices[i])**2).mean()
                                for i,p in enumerate(x_slices))
                cond_reg   = sum(((p - c0_slices[i])**2).mean()
                                for i,p in enumerate(c_slices))
                prior_loss = lambda_latent * (latent_reg + cond_reg)

                # 5) Total loss
                total_loss = (lambda_roi * total_roi_loss
                            + lambda_smooth * smooth_loss
                            + prior_loss)

            # 6‑2) backward (mixed‑precision)
            scaler.scale(total_loss).backward()

            # 6‑3) stateless functional update (slice별 완전 분리)
            for i in range(N):
                grads = (x_slices[i].grad, c_slices[i].grad)
                updates, opt_states[i] = func_opt.update(grads, opt_states[i])
                new_x, new_c = apply_updates((x_slices[i], c_slices[i]), updates)
                x_slices[i].data.copy_(new_x.data)
                c_slices[i].data.copy_(new_c.data)
                x_slices[i].grad = None
                c_slices[i].grad = None

            # # 6‑4) GradScaler 업데이트
            # scaler.update()

            # 6‑5) slice‑wise Early Stopping
            with torch.no_grad():
            ...
            # 6‑6) Projection Trick (batch‑wise)
            with torch.no_grad():
                for i, p in enumerate(x_slices):
                    p.data = (1 - alpha_proj) * p.data + alpha_proj * x0_slices[i]
                for i, p in enumerate(c_slices):
                    p.data = (1 - alpha_proj) * p.data + alpha_proj * c0_slices[i]

            # 6‑7) 로그 출력 & 종료 조건
            print(f"[Epoch {epoch+1}/{num_epoch}] "
                f"ROI={total_roi_loss.item():.6f}  "
        ...
        save_path_a = os.path.join(
            save_dir_a,
            f"MR_Heart_{sid}_frame_{frame_idx}_final_100_{new_n}slice.nii.gz"
        )
