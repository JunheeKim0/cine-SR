...
# 834, 
    # 148, 400, 769, 243, 813, 600, 100, 708,
...
def main(local_rank, world_size):
    ...
    my_ids = id_list[::-1][local_rank::world_size]
    ...
    for sid in my_ids:
        for frame_idx in n_frames:
            ...
            sax_interp_path = (
                ...
                f"PNUH_cine_LAX_SAX_align_2/MR_Heart_{sid}/sax_resample_linear_cropped/sax_resample_linear_cropped_{frame_idx}.nii.gz"
            )
     ...
     ################### ======== Slice별 최적화 시작 (new_n 개수만큼) ========= ######################
        ...
        ################### --- 최적화 완료 후 예측 저장 (각 보간된 slice별) --- #######################
            ...
            save_dir_3d = (
                f"/workspace/storage/kjh/dataset/cardiac/output/Inverse/val"
