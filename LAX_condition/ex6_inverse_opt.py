...
def optimize_sax(
    ...
    frame: int,
    device: torch.device,
    lambda_roi:    float = 5.0,
    lambda_prior:  float = 1.0,
    lambda_smooth: float = 3.0,
    lambda_latent: float = 0.02,
    alpha_proj: float = 0.001,
    # … 필요하면 other hyperparams
):
    """
    - subject_id, frame: 데이터를 지정
    - device: GPU 지정
    - lambda_* : 가중치(λ)들
    → 최종에 2ch/3ch/4ch projection 결과만 return
    """
    ...
    for id in id_list : 
        ...
        for ch in channels:
            ...
            for var in variants:
                ...
                fpath_img  = os.path.join(f"/workspace/storage/icml_data_collection/Cardiac_database/PNUH/PNUH_cine_LAX_SAX_align_2/MR_Heart_{subject_id}", fname_img_full)
                fpath_mask = os.path.join(f"/workspace/storage/icml_data_collection/Cardiac_database/PNUH/PNUH_cine_LAX_SAX_align_2/MR_Heart_{subject_id}", fname_mask_full)

                if os.path.exists(fpath_img) and os.path.exists(fpath_mask):
                    # 확장자 제거
                    fname_img = fpath_img
                    fname_mask = fpath_mask

                    gt_path  = os.path.join(fname_img, f'{ch}_{var}_1lax_cropped_{frame}.nii.gz')
                    mask_path= os.path.join(fname_mask, f'{ch}_{var}_1lax_mask_cropped_adjusted_{frame}.nii.gz')
                    proj_gt, m = load_lax_mask(gt_path, mask_path, device)
                    masks[ch] = m
        ...
        for ch in ['2ch','3ch','4ch']:
            preds[ch] = projection_torch(vol_pred, masks[ch])
        return preds

if __name__=="__main__":
    # torchrun --nproc_per_node=4 LA_condition_all_pred_after.py
    preds = optimize_sax(subject_id=400, frame=0, device=torch.device('cuda:0'))
    for ch, img in preds.items():
        print(f"{ch} projection shape: {img.shape}")
