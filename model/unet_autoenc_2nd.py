...
class BeatGANsAutoencModel(BeatGANsUNetModel):
    def __init__(self, conf: BeatGANsAutoencConfig):
        super().__init__(conf)
        self.conf = conf
        # ADD: choose insertion point index (e.g., after block k)
        self.where_to_insert = getattr(conf, 'insert_block_idx', 5)  # 수정: where_to_insert 속성 추가        
        ...
        if x is not None:
            ...
            for i in range(len(self.input_num_blocks)):
                for j in range(self.input_num_blocks[i]):
                    ...
                    if k == self.where_to_insert and seg_style is not None:                    
                        seg_style=self.seg_style_conv(seg_style) #Pass through 1*1 conv
                        h=self.input_blocks[k](h,extra=seg_style,emb=emb,cond=enc_cond_emb)
                        #print("!!!!!!!!!!CONCAT!!!!!!!!!!!!!")
                    else:
