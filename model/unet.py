...
class BeatGANsUNetModel(nn.Module):
    ...
    def forward(self, x, t, y=None, **kwargs):
        ...
        k = 0
        for lvl in range(len(self.input_num_blocks)):
            for _ in range(self.input_num_blocks[lvl]):
                block = self.input_blocks[k]
                # seg_style concat 구간만 extra 인자 포함
        ...
        # output slice
        k = 0
        for lvl in range(len(self.output_num_blocks)):
            for _ in range(self.output_num_blocks[lvl]):
                block = self.output_blocks[k]
                try:
                    lateral = skips[-lvl-1].pop()
                except IndexError:
                    lateral = None
