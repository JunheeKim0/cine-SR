...
class BeatGANsUNetConfig(BaseConfig):
    ...
    where_to_concat_seg_cond = 4#Changed ARIST 
    label_channel_num = 512 #Changed ARIST
...
class BeatGANsUNetModel(nn.Module):
    ...
    def forward(self, x, t, seg_style, y=None, **kwargs):
        """
        ...
        h = x.type(self.dtype)
        skips = [[] for _ in range(len(self.conf.channel_mult))]
        k = 0
        for level in range(len(self.input_num_blocks)):
            for _ in range(self.input_num_blocks[level]):
                block = self.input_blocks[k]
                # concat seg_style if needed
                if k == self.where_to_insert:
                    # block(h, extra=seg_style, emb=emb)
                    h = checkpoint(block, h, seg_style, emb)
                else:
                    h = checkpoint(block, h, emb)
                skips[level].append(h)
                k += 1
        
        h = checkpoint(self.middle_block, h, emb)

        k = 0
        for level in range(len(self.output_num_blocks)):
            for _ in range(self.output_num_blocks[level]):
                block = self.output_blocks[k]
                # lateral skip connection
                try:
                    lateral = skips[-level-1].pop()
                except IndexError:
                    lateral = None
                # block(h, emb=emb, lateral=lateral)
                h = checkpoint(block, h, emb, lateral)
                k += 1
...
class BeatGANsEncoderModel(nn.Module):
    ...
    def __init__(self, conf: BeatGANsEncoderConfig):
        ...
        ch = int(conf.channel_mult[0] * conf.model_channels) #HARD CODED conf.model_channels
        self.input_blocks = nn.ModuleList([
            TimestepEmbedSequential(
...
# ## CHANGED ARIST
# class BeatGANsEncoderModel_COND(nn.Module):
#     """
#     The half UNet model with attention and timestep embedding.
