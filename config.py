...
data_paths = {
    ...
    'medical_dataset':
    r"D:\data\Cardiac\Cine\DICOM\test"
}
...
class TrainConfig(BaseConfig):
    ...
    def make_dataset(self, path=None, **kwargs):
        if self.data_name == 'ffhqlmdb256':
            ...
            return CelebAlmdb(path=path or self.data_path,
                              ...
                              crop_d2c=True,
                              **kwargs)
        elif self.data_name == 'medical_dataset':
            return MedicalDataset(path="/workspace/storage/kjh/dataset/cardiac/UK_bio/128_cine/train",image_size=self.img_size,**kwargs)
        elif self.data_name== 'medical_dataset_latent':
            return MedicalDatasetWithLatent(path="/workspace/storage/kjh/dataset/cardiac/UK_bio/128_cine/train/img",image_size=self.img_size,path_to_latent="/workspace/storage/kjh/dataset/cardiac/UK_bio/128_cine/train/latent",**kwargs)
        elif self.data_name== 'medical_dataset_label':
            return MedicalDatasetWithLabel(path="/workspace/storage/kjh/dataset/cardiac/UK_bio/128_cine/train",image_size=self.img_size)
        else:
            raise NotImplementedError()
    ...
    def make_loader(self,
                    ...
                    batch_size: int = None,
                    parallel: bool = False):
        # Windows 환경 대응: fork 대신 spawn 사용
        # ctx = get_context('spawn')
        # ctx = mp.get_context('forkserver')
        ctx = mp.get_context()
        if parallel and distributed.is_initialized():
            # Distributed 환경에서는 샘플러 사용
            sampler = DistributedSampler(dataset,
                                         shuffle=shuffle,
                                         drop_last=drop_last)
            shuffle_flag = False
        ...
        return DataLoader(
            ...
            prefetch_factor=1,            # 한 번에 가져오는 샘플 수 낮춤
            multiprocessing_context=ctx,
            drop_last=drop_last,
        )    
    ...
    def make_model_conf(self):
        ...
        if self.model_name == ModelName.beatgans_ddpm:
            ...
            self.model_conf = cls(
                ...
                ModelName.beatgans_autoenc_2nd,
            ]:
            print("why...?")
            from model.unet_autoenc_2nd import BeatGANsAutoencConfig
            cls = BeatGANsAutoencConfig
            # supports both autoenc and vaeddpm
            if self.model_name == ModelName.beatgans_autoenc_2nd:
                self.model_type = ModelType.autoencoder
            else:
                raise NotImplementedError()
            ...
            self.model_conf = cls(
                ...
                ModelName.beatgans_autoenc_E2E,
        ]:
            from model.unet_autoenc_seg_e2e import BeatGANsAutoencConfig
            cls = BeatGANsAutoencConfig
            print("E2E")
            # supports both autoenc and vaeddpm
            if self.model_name == ModelName.beatgans_autoenc_E2E:
                self.model_type = ModelType.autoencoder
            else:
                raise NotImplementedError()
