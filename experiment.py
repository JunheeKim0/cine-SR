...
class LitModel(pl.LightningModule):
    ...
    def render(self, noise, cond=None, T=None):
        ...
        if cond is not None:
            # 체크포인트 적용
            def _rc(noise_t, cond_t):
                out = render_condition(
                    self.conf, self.ema_model,
                    noise_t, sampler=sampler, cond=cond_t)
                return (
                    out["sample"],
                    out["pred_xstart"],
                    out["pred_noise"],
            ...
            # pred_img = checkpoint(_rc, noise, cond)
            sample, pred_xstart, pred_noise, pred_sigma = checkpoint(_rc, noise, cond)
            pred_img = {
                "sample": sample,
            ...
            def _ru(noise_t):
                return render_uncondition(
                    self.conf, self.ema_model,
                    noise_t, sampler=sampler, latent_sampler=None)
            # 입력이 noise 하나뿐이므로 args 튜플로 넘겨야 합니다.
            pred_img = checkpoint(_ru, noise)
    ...
    def infer_whole_dataset(self,
        ...
        with writer:
            for batch in tqdm.tqdm(loader, total=len(loader), desc='infer'):
                # with torch.no_grad():
                print("what???_infer_whole_dataset")
                # (n, c)
...
def train(conf: TrainConfig, gpus, nodes=1, mode: str = 'train'):
    ...
    checkpoint_path = f'{conf.logdir}/epoch=11-step=435000.ckpt'
    ...
    if len(gpus) == 1 and nodes == 1:
        accelerator = 'gpu'
        devices = gpus
        strategy = None    # single-GPU/CPU 에선 전략 없음
    else:
        accelerator = 'gpu'
        devices = gpus
        strategy = DDPStrategy(find_unused_parameters=False)  # DDP 전략
    ...
    #     accelerator=accelerator,
    #     # precision=16 if conf.fp16 else 32,
    #     precision="16-mixed" if conf.fp16 else 32,  # use recommended AMP setting
    #     callbacks=[
    #         checkpoint,
    #         LearningRateMonitor(),
    #     ],
