    ...
    with torch.no_grad():
        ...
        for batch in tqdm(val_loader, desc='lpips'):
            ...
            if use_inverted_noise:
                ...
                x_T = torch.randn((len(imgs), 1, conf.img_size, conf.img_size), #CHANGED ARIST
                                  device=device)
            ...
            if conf.model_type == ModelType.ddpm:
                ...
                if conf.dataset_seg_latent == True:
                    assert conf.model_type.has_autoenc()
                    global_semantic_code = model.encode(imgs)["cond"]
                    pred_imgs = render_condition(conf=conf,
                                                model=model,
                                                x_T=x_T,
                                                x_start=imgs,
                                                cond=global_semantic_code,
                                                seg_style=cond, # CHANGED ARIST
                                                #Our methods, the input cond is seg style, need to produce cond
                                                sampler=sampler)
                else:
                    pred_imgs = render_condition(conf=conf,
                            model=model,
        ...
        if conf.train_mode.is_latent_diffusion():
            ...
            for batch in tqdm(train_loader, desc='generating images'):
                ...
                batch_images = render_condition(
                    ...
                    x_start=imgs,
                    cond=cond,
                    sampler=sampler).cpu() #CHANGED ARIST
                # model: BeatGANsAutoencModel
                # # returns {'cond', 'cond2'}
                # conds = model.encode(imgs)
