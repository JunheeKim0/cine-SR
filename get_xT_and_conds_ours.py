...
base_path=r"D:\data\Cardiac\Cine\DICOM\test"             # Nifti  (3D+T, 하나의 폴더에 subject들 subjXXX.nii.gz)
path_to_latent=r"D:\data\Cardiac\Cine\DICOM\test"     # 아마 segmentation numpy (각 슬라이스마다 추출 XXX_X.npy)
...
for i in tqdm(files):
    ...
    # segmentation encoding (E_mor)
    conds = model.encode(latents_tensor)

    # image encoding (E_sem)
